...
function getEstados(){
	...
	jQuery.ajax({
		type: "POST",
		url: getServer()+"/server/modulos/mod_candidato/getEstados.php",
		success: function(data){

				for(i=0;i<data.length;i++){
					
					retorno.push(data[i].estado_user_cand);
...
function getCidade(estado){
	...
	jQuery.ajax({
		type: "POST",
		data: dados,
		url: getServer()+"/server/modulos/mod_candidato/getCidades.php",
		success: function(data){

				for(i=0;i<data.length;i++){
					
					retorno.push(data[i].cidade_user_cand);
...
function getBairro(cidade){
	...
	jQuery.ajax({
		type: "POST",
		data: dados,
		url: getServer()+"/server/modulos/mod_candidato/getBairros.php",
		success: function(data){

				for(i=0;i<data.length;i++){
					
					retorno.push(data[i].bairro_user_cand);
